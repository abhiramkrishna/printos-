  ...
  return (
    <header 
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        isScrolled ? 'bg-white/95 backdrop-blur-xl shadow-md' : 'bg-white/90 backdrop-blur-sm shadow-sm'
      }`}
    >
    ...
    <div className="relative h-[300px] sm:h-[450px] lg:h-[650px] w-full rounded-[2rem] sm:rounded-[3rem] overflow-hidden shadow-2xl border-[6px] sm:border-[10px] border-white group">
      {HERO_SLIDES.map((slide, index) => (
        <img
          ...
          className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 ease-in-out transform md:group-hover:scale-105 ${
        ...
        {HERO_SLIDES.map((_, index) => (
          <button
            ...
            className={`h-2 sm:h-2.5 rounded-full transition-all duration-300 ${
              index === current ? 'bg-white w-6 sm:w-8' : 'bg-white/50 hover:bg-white/80 w-2 sm:w-2.5'
            }`}
        ...
        <div className="space-y-1">
          ...
          <div className="mb-2">
            <button 
              ...
              className={`w-full flex items-center justify-between px-3 py-2.5 sm:px-4 sm:py-3 rounded-[14px] transition-colors active:scale-[0.98] ${
                activeCategory === 'All' ? 'bg-gray-100/80 font-bold text-gray-900' : 'hover:bg-gray-50 text-gray-600 font-medium'
              }`}
            >
                ...
                return (
                  <div key={cat.id} className="relative">
                    ...
                    <button 
                      ...
                      className={`w-full text-left py-2 px-2 sm:px-3 rounded-lg text-[13px] sm:text-sm transition-colors flex items-center justify-between active:scale-[0.98] ${
                        activeCategory === cat.id && !activeSubCategory && !searchTerm ? 'text-gray-900 font-bold bg-gray-50' : 'text-gray-500 hover:text-gray-900 font-medium'
                      }`}
                    >
                      ...
                      <div className="pl-4 sm:pl-5 pb-2 space-y-1 mt-1 animate-fade-in">
                        {cat.subItems.map(sub => (
                          <button
                            ...
                            className={`w-full text-left py-1.5 px-2 rounded-md text-xs sm:text-[13px] transition-colors truncate active:scale-[0.98] ${
                              activeSubCategory === sub && !searchTerm ? 'text-blue-600 font-bold bg-blue-50/50' : 'text-gray-500 hover:text-gray-900 font-medium'
                            }`}
                          >
                            {sub}
                          </button>
...
const ProductDetails = ({ product, navigateTo, setActiveCategory, setActiveSubCategory }) => {
    ...
    const handleCopyLink = () => {
        ...
        const currentUrl = window.location.href.includes('localhost') || window.location.href.includes('preview') ? dummyUrl : window.location.href;
                    ...
                    <div className="border-t border-gray-100 pt-5 sm:pt-6 space-y-2 sm:space-y-3 text-xs sm:text-sm">
                        ...
                        <div className="flex items-center">
                            ...
                            <div className="flex space-x-4 text-gray-400 relative">
                                <button type="button" onClick={handleNativeShare} aria-label="Share on Instagram">
                                    <Instagram className="w-4 h-4 hover:text-pink-600 cursor-pointer active:scale-90 transition-all" />
                                </button>
                                <a href={getWhatsappShareLink()} target="_blank" rel="noreferrer" aria-label="Share on WhatsApp">
                                    <WhatsappIcon className="w-4 h-4 hover:text-green-500 cursor-pointer active:scale-90 transition-all" />
                                </a>
                                <button type="button" onClick={handleCopyLink} className="relative" aria-label="Copy link">
                        ...
                        {relatedProducts.map(relProduct => (
                            ...
                            >
                              ...
                              <div className="flex-1 flex flex-col w-full">
                                 ...
                                 <h3 className="font-bold font-poppins text-gray-900 text-lg sm:text-xl leading-tight mb-3 line-clamp-1 md:group-hover:text-blue-600 transition-colors">
                                     {relProduct.name}
                                 </h3>
                                 <div className="mt-auto flex flex-col gap-5">
                                     <div className="flex flex-row items-center justify-between">
